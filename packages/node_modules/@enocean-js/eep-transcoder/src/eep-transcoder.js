...
export function eep2IP (eep) {
  var eepDesc = getEEP(eep)
  var funcs = eepDesc.case[0].datafield.reduce((acc, item) => {
  ...
  return c.case.find(function (item) {
    switch (getConditionType(item)) {
      case 'status':
        ...
        break
      case 'data':
        var cond = selector.payload.getValue(parseInt(item.condition.datafield.bitoffs), parseInt(item.condition.datafield.bitsize))
  ...
  return function (returnValue, datafield, index, array) {
    ...
    switch (getFieldType(datafield)) {
      case 'enum':
        ...
        break
      case 'spread':
        var v = datafield.spread.reduce((accumulator, current) => {
...
function extractEnumValue (value, list) {
  ...
  return list.find(item => {
    ...
    if ('bitmask' in item) {
      var mask = parseInt(item.bitmask, 2)
...
function getPayloadLengthFromEEPDescriptor (desc) {
  ...
  var res = desc.datafield.map(field => {
    if ('spread' in field) {
...
export function encodeData (data, options) {
  ...
  var payloadLength = getPayloadLengthFromEEPDescriptor(desc)
  var diff = payloadLength - payload.length
  if (diff > 0) {
    for (var l1 = 0; l1 < diff; l1++) {
      payload.unshift(0)
    }
  } else {
    for (var l2 = 0; l2 < -diff; l2++) {
      payload.pop()
    }
  }
  ...
  for (var name in data) {
    ...
    if (field) {
      ...
      if (fieldType === 'range' || fieldType === 'spread') {
        var v = Math.round(setRangeValue(val, field, desc.datafield, payload))
        if (fieldType === 'spread') {
          var totalLength = field.spread.reduce((acc, current) => acc + current.bitsize)
          var valueString = v.toString(2).padStart(totalLength, '0')
          field.spread.forEach(current => {
            var curentValue = valueString.substr(0, current.bitsize)
...
export function getTeachInInfo (telegram) {
  ...
  if (p.RORG === 0xd4) {
    ...
    var msb = p.payload.getValue(29, 3).toString(2).padStart(3, '0')
    var lsb = p.payload.getValue(16, 8).toString(2).padStart(3, '0')
